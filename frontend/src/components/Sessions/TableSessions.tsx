...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/sessions/sessionsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureSessionsCols';
...
const TableSampleSessions = ({
  ...
  useEffect(() => {
    if (sessionsNotify.showNotification) {
      notify(sessionsNotify.typeNotification, sessionsNotify.textNotification);
    }
  }, [sessionsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/sessions/sessions-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/sessions/sessions-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          pathEdit={`/sessions/sessions-edit/?id=`}
          pathView={`/sessions/sessions-view/?id=`}
...
export default TableSampleSessions;
